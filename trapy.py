...
def recv(conn:Conn, length:int):
    ...
    while not fin and len(big_data) < length:
        ...
        data, secnum, acknum, tcp_flags, source_addr, source_port = conn.decode_packet(packet)

        print(f"Recieved packet({len(packet)}):", data)
        print(f"From: {source_addr}:{source_port}")

        if conn.dest_hostport == None:
            conn.dest_hostport = (source_addr, source_port)
        elif conn.dest_hostport != (source_addr, source_port):
            print(f"Recieving another connection from {source_addr}:{source_port}. Was especting {conn.dest_hostport[0]}:{conn.dest_hostport[1]} ignoring for now.")
        ...
        if conn.connecting and "syn" in tcp_flags and "ack" in tcp_flags:
            conn.acknum = secnum + 1
            return tcp_flags
        if conn.connecting and "ack" in tcp_flags:
            conn.acknum = secnum + 1
            return tcp_flags
        ...
        if conn.data_transm and "ack" in tcp_flags:
            if secnum == conn.acknum:
                ...
                print(f"Was waiting {conn.acknum} instead {secnum}.")
