...
from utils import parse_flags, address_to_bytes, calculate_checksum, from_bytes_to_flags
...
class Conn:
    ...
    def make_packet(self, data:bytes=b"", tcp_flags:str="") -> bytes:
        def make_ip_header():
            ...
            ttl = b'\x40'
            protocol = b'\x06'
            source_address = address_to_bytes(self.socket.getsockname()[0])
            dest_address = address_to_bytes(self.dest_hostport[0])#(self.socket.getpeername()[0])
    ...
    def decode_packet(self, packet:bytes) -> Tuple[bytes, int, int, Set[str]]:
        ...
        def decode_ip_header(ip_header:bytes):
            ...
            sourceaddress = ip_header[12:16]
            destaddress = ip_header[16:20]
        ...
        def decode_tcp_header(tcp_header:bytes):
            ...
            return secnumber, acknumber, flags2
        ...
        #decode_ip_header(ip_header)
        secnumber, acknumber, flags = decode_tcp_header(tcp_header)
        return data, secnumber, acknumber, flags
